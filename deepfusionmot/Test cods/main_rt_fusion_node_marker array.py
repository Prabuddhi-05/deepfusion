...
class RealTimeFusionNode(Node):
    def __init__(self, cfg):
        ...
        # Publisher for tracking results (MarkerArray for RViz)
        self.tracking_pub = self.create_publisher(MarkerArray, '/tracking_markers', 10)

        # Optional: Timer-based approach
        # self.timer_ = self.create_timer(0.1, self.process_frame)  # e.g., 10 Hz
    ...
    def lidar_callback(self, msg):
        ...
        self.latest_pc = msg  # Convert to a NumPy array if needed
    ...
    def process_frame(self):
        ...
        # Combine results for both classes
        if trackers_car is not None and trackers_ped is not None and \
           trackers_car.size > 0 and trackers_ped.size > 0:
