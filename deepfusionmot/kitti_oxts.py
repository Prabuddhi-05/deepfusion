...
from deepfusionmot.coordinate_transformation import TransformationKitti
from deepfusionmot.file import fileparts
...
def egomotion_compensation_ID(traj_id, calib_file, ego_rot_imu, ego_xyz_imu, left, right, mask=None):
    # traj_id           # N x 3
    ...
    traj_id_imu = TransformationKitti(calib_file).rect_to_imu(traj_id)  # less_pre x 3
    ...
    # correct transition
    if mask is not None:
        traj_id_imu += ego_xyz_imu[good_index, :]  # les_frames x 3, TODO, need to testing which is correct
    else:
    ...
    traj_id_rect = TransformationKitti(calib_file).imu_to_rect(traj_id_imu)
