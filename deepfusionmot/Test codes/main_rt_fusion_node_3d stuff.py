...
# If you need to republish the LiDAR as a new topic, you'll convert your self.latest_pc
# from sensor_msgs/PointCloud2 to another PointCloud2 (which is basically the same).
# Or just copy the original message to the new topic.
...
class RealTimeFusionNode(Node):
    def __init__(self, cfg):
        ...
        # NEW: Republish LiDAR data, plus a dedicated MarkerArray for 3D bounding boxes
        self.lidar_points_pub = self.create_publisher(PointCloud2, '/lidar_points_detections', 10)
        self.lidar_detections_pub = self.create_publisher(MarkerArray, '/lidar_detections_markers', 10)
    ...
    def process_frame(self):
        ...
        # Category-level
        trackers_car = self.run_fusion_and_update(self.tracker_car, self.detections_2d_car, self.detections_3d_car, 'Car')
        trackers_ped = self.run_fusion_and_update(self.tracker_ped, self.detections_2d_ped, self.detections_3d_ped, 'Pedestrian')

        # Combine
        if trackers_car is not None and trackers_car.size>0 and trackers_ped is not None and trackers_ped.size>0:
        ...
        # 1) Publish final bounding boxes as before
        marker_array = self.create_marker_array(combined_trackers)
        self.tracking_pub.publish(marker_array)
        self.get_logger().info(f"Published {len(marker_array.markers)} markers for visualization (tracking_markers).")

        # 2) Publish annotated image
        self.publish_annotated_image()

        # 3) Publish LiDAR with 3D bounding boxes from 2D detections
        #    e.g. If you have a way to convert 2D boxes -> 3D boxes, or final track 3D boxes.
        self.publish_lidar_with_detections()
    ...
    def run_fusion_and_update(self, tracker, dets_2d, dets_3d, category_name):
        ...
        dets_3d_fusion, dets_3d_only, dets_2d_only = data_fusion(
            dets_3d_camera, dets_2d_frame, dets_3dto2d_image, additional_info
        )
        self.get_logger().info(f"Fusion results for {category_name}: {len(dets_3d_fusion['dets_3d_fusion'])} fused, {len(dets_2d_only)} 2D-only, {len(dets_3d_only['dets_3d_only'])} 3D-only.")
    ...
    def publish_lidar_with_detections(self):
        """
        Re-publish the LiDAR point cloud plus a new MarkerArray showing
        3D bounding boxes derived from your 2D detections or final tracker.
        """
        # 1) Republish LiDAR (if available)
        if self.latest_pc is not None:
            # We'll just forward the same message or transform it if needed
            # Example: no modifications
            self.lidar_points_pub.publish(self.latest_pc)
        else:
            return  # no point cloud yet

        # 2) Build a new MarkerArray for bounding boxes from the 2D detection in 3D
        # If you do not have a direct method to convert 2D->3D bounding boxes,
        # you might rely on final tracker outputs. For simplicity, let's assume
        # we do have some 3D boxes from the final tracker or partial fusion.
        # We'll just reuse create_marker_array(...) for demonstration.
        # In real code, you might have separate logic or partial boxes in 3D.
        
        # Suppose you want to use the final trackers of 'Car' + 'Ped' or store them
        # from process_frame. We'll just pretend we do the same call:
        # "combined_trackers" from the last process_frame step. 
        # For clarity, let's store it in a buffer if needed. For now, do a minimal example:

        # We'll call create_marker_array with an empty array or your final array.
        # If you need the final array, you can store it as self.tracker_output_final
        # in process_frame. For demonstration, let's just do an empty for now:
        # marker_array_3d = self.create_marker_array(self.tracker_output_final)
        # We'll do a minimal version:

        marker_array_3d = MarkerArray()
        # Example: create a single dummy bounding box for demonstration
        # Typically you'd loop over final trackers or 2D->3D boxes
        marker = Marker()
        marker.header.stamp = self.get_clock().now().to_msg()
        marker.header.frame_id = "velodyne"  # same as LiDAR
        marker.ns = "lidar_detections"
        marker.id = 999
        marker.type = Marker.CUBE
        marker.action = Marker.ADD
        marker.lifetime = rclpy.duration.Duration(seconds=0).to_msg()
        # Some random bounding box (x=5, y=0, z=0, etc.)
        ...
        marker.color.b = 0.0
        marker.color.a = 0.6
        marker_array_3d.markers.append(marker)

        self.lidar_detections_pub.publish(marker_array_3d)
        self.get_logger().info("Published LiDAR points + custom 3D boxes on new topics (/lidar_points_detections, /lidar_detections_markers).")
