...
class RealTimeFusionNode(Node):
    def __init__(self, cfg):
        ...
        # 5) Calibration data as a std_msgs/String with custom QoS
        self.create_subscription(String, '/camera/calibration', self.calibration_cb, 1)
        ...
        self.get_logger().info("RealTimeFusionNode initialized. Waiting for detections and calibration data...")
    ...
    def calibration_cb(self, msg: String):
        ...
        # For debugging: Log the keys of the parsed calibration data
        self.get_logger().debug(f"Parsed calibration keys: {list(self.calib_data.keys())}")
        # Optionally, log the calibration matrices
        # for key, matrix in self.calib_data.items():
