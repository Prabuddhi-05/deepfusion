...
class RealTimeFusionNode(Node):
    def __init__(self, cfg):
        ...
        self.latest_pc = None  # If you want to store point clouds
        ...
        # Publisher for 3D tracking markers in RViz
        self.tracking_pub = self.create_publisher(MarkerArray, '/tracking_markers', 10)
        # NEW: Publisher for the annotated camera image
    ...
    def process_frame(self):
        ...
        # Combine results
        if trackers_car is not None and trackers_ped is not None \
           and trackers_car.size > 0 and trackers_ped.size > 0:
        ...
        # 2) Publish a 2D-annotated image with bounding boxes
    ...
    def publish_annotated_image(self):
        """Draw 2D bounding boxes on the latest_image and publish it."""
        if self.latest_image is None:
            return
        annotated = self.latest_image.copy()

        # (A) Draw 2D Car boxes in green
        ...
        # (B) Draw 2D Pedestrian boxes in blue
        for det in self.detections_2d_ped:
            # format: [frame, x1, y1, x2, y2, score]
            x1, y1, x2, y2 = map(int, det[1:5])
        ...
        annotated_msg.header.frame_id = "camera_link"  # or whichever frame your camera uses

        # Publish annotated image
        self.annotated_image_pub.publish(annotated_msg)

    def create_marker_array(self, trackers):
        """Convert tracker outputs to an RViz MarkerArray of 3D bounding boxes."""
...
def main(args=None):
    # Hard-coded absolute path to your YAML config:
